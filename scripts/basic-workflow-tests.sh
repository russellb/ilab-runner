...
test_train() {
    ...
    if [ "$TRAIN_LIBRARY" -eq 1 ]; then
        ...
        TRAIN_ARGS=("--device=cuda" "--model-path=instructlab/granite-7b-lab" "--data-path=${SCRIPTDIR}/test-data/train_all_pruned_SDG.jsonl" "--lora-quantize-dtype=nf4" "--4-bit-quant")
        if [ "$GRANITE" -eq 1 ]; then
